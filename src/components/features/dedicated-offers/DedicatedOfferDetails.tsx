...
export default function DedicatedOfferDetails({
  ...
  const { statusUpdateLoading } = useSelector(
    (state: RootState) => state.dedicated-offers
  );
  ...
  const handleApprove = () => {
    dispatch(updateDedicatedOfferStatusStart({ id: dedicated-offerId, status: "Approved" }));
  };
  ...
  return (
    <div>
      {dedicated-offer.offer_status === "Draft" &&
        dedicated-offer.account_status === "Pending" && (
      ...
      <TabGroup selectedIndex={selectedIndex} onChange={handleTabChange}>
        ...
        <TabPanels className="md:px-4">
          {tabs.map((tab) => (
            <TabPanel key={tab}>
              {tab === "Creators" && <Creators dedicated-offer={dedicated-offer} />}
              {tab === "Overview" && <Overview dedicated-offer={dedicated-offer} />}
              {tab === "Availabilites" && (
                <Availabilites dedicated-offerId={dedicated-offerId} />
              )}
              {tab === "Posts" && <Posts />}
              {tab === "Reviews" && <Reviews dedicated-offerId={dedicated-offerId} />}
