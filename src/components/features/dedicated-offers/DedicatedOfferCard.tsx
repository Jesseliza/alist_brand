...
interface DedicatedOfferCardProps {
  dedicated-offer: DedicatedOfferDisplay;
  checked: boolean;
...
export default function DedicatedOfferCard({
  ...
  const isDedicatedOfferActive = () => {
    const now = new Date();
    const startDate = new Date(dedicated-offer.startDate);
    const endDate = new Date(dedicated-offer.endDate);
    return now >= startDate && now <= endDate;
  };

  const getModeIcon = () => {
    const active = isDedicatedOfferActive();
    if (dedicated-offerType === "WalkIn") {
      return active
        ? "/icons/dedicated-offer/card/walk-approved.svg"
        : "/icons/dedicated-offer/card/walk-pending-light.svg";
    } else if (dedicated-offerType === "Delivery") {
      return active
  ...
  const handleRemoveClick = (e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    onRemove(dedicated-offer.id.toString());
  };
        ...
        <div className="grid grid-cols-3 gap-[9px] mb-[13px]">
          ...
          <div className="aspect-square flex flex-col justify-center items-center gap-2 rounded-[11px] bg-white shadow-[0_0_2px_rgba(0,0,0,0.16)]">
            <div className="h-[30.65px] flex items-center justify-center">
              <Image
                src={getModeIcon()}
                alt={dedicated-offerType || "DedicatedOffer"}
                width={dedicated-offerType === "WalkIn" ? 18.04 : 31.87}
                height={dedicated-offerType === "WalkIn" ? 29.65 : 30.65}
              />
            </div>
