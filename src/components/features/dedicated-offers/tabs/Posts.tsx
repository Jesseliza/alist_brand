import { useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import { useParams } from "next/navigation";
import { getReviewPostsStart } from "@/store/dedicated-offers/DedicatedOfferSlice";
import { RootState } from "@/store/store";
import DedicatedOfferDetailsPost from "./Posts/DedicatedOfferDetailsPost";
import Pagination from "../../../general/Pagination";
import Loader from "@/components/general/Loader";
import { DedicatedOfferReviewPost } from "@/types/entities";

const formatNumber = (num: number | null) => {
  if (num === null) return "0";
  if (num >= 1000) {
    return `${(num / 1000).toFixed(1)}K`;
  }
  return num.toString();
};

export default function Posts() {
  const dispatch = useDispatch();
  const params = useParams();
  const { dedicated-offerId } = params;

  const {
    reviewPosts,
    reviewPostsLoading,
    reviewPostsError,
    reviewPostsPagination,
  } = useSelector((state: RootState) => state.dedicated-offers);

  useEffect(() => {
    if (dedicated-offerId) {
      dispatch(getReviewPostsStart({ id: dedicated-offerId as string }));
    }
  }, [dispatch, dedicated-offerId]);

  const handlePageChange = (page: number) => {
    if (dedicated-offerId) {
      dispatch(
        getReviewPostsStart({
          id: dedicated-offerId as string,
          page,
          per_page: reviewPostsPagination?.per_page,
        })
      );
    }
  };

  const handleItemsPerPageChange = (items: number) => {
    if (dedicated-offerId) {
      dispatch(
        getReviewPostsStart({
          id: dedicated-offerId as string,
          page: 1,
          per_page: items,
        })
      );
    }
  };

  if (reviewPostsLoading) {
    return <Loader />;
  }

  if (reviewPostsError) {
    return <p className="text-red-500 text-center py-8">Error: {reviewPostsError}</p>;
  }

  const mappedPosts = reviewPosts.map((post: DedicatedOfferReviewPost) => {
    const imageUrlBase = process.env.NEXT_PUBLIC_IMAGE_URL || "";
    const postImages = [
      post.screenshot1,
      post.screenshot2,
      post.screenshot3,
      post.screenshot4,
    ]
      .filter((img): img is string => img !== null)
      .map((img) => `${imageUrlBase}/assets/uploads/userreviews/${img}`);

    const authorImage = post.user.profile_picture
      ? `${imageUrlBase}${post.user.profile_picture}`
      : null;

    return {
      authorImage,
      authorName: post.user.name,
      instagramUsername: post.user.instagram_url || "N/A",
      stats: {
        followers:
          post.user.instagram_follower_range?.followers ||
          formatNumber(post.user.instagram_followers),
        reach: post.user.instagram_follower_range
          ? formatNumber(post.user.instagram_follower_range.reach)
          : formatNumber(post.rating),
        engagement: "0%", // Not available in the response
      },
      postImages: postImages,
    };
  });

  return (
    <div>
      <div className="max-w-[1272px] mx-auto md:mt-[60px] mt-[14px] ">
        <div
          className="grid grid-cols-1 gap-[13px] justify-center"
          style={{
            gridTemplateColumns: "repeat(auto-fit, minmax(360px, 412px))",
          }}
        >
          {mappedPosts.map((post, index) => (
            <DedicatedOfferDetailsPost key={index} {...post} />
          ))}
        </div>

        {reviewPostsPagination && reviewPostsPagination.total > 0 && (
          <div className="mt-8">
            <Pagination
              totalItems={reviewPostsPagination.total}
              itemsPerPage={reviewPostsPagination.per_page}
              currentPage={reviewPostsPagination.current_page}
              onPageChange={handlePageChange}
              onItemsPerPageChange={handleItemsPerPageChange}
              fixed={false}
            />
          </div>
        )}
      </div>
    </div>
  );
}
